...
- If the browser drops the WebGL context later (a GPU reset, or too many live contexts), the gradient returns at once and the shader is rebuilt on a fresh canvas. If the context is lost again within 10 seconds, the gradient stays.
